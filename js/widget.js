...
function getusd() {
    ...
    return $.getJSON('https://index-api.bitcoin.com/api/v0/core/price/usd');
  
}


$.when(getusd()).then(function (data) {
    usdPrice = data.price;
  

function getgbp() {
    
    return $.getJSON('https://index-api.bitcoin.com/api/v0/core/price/gbp');
  
  }
  
  $.when(getgbp()).then(function (data) {
    gbpPrice = data.price;
    

convRate = (gbpPrice / usdPrice).toFixed(3);
                        ...
                        lang: r(function(t) {
                            return dt.test(t || "") || e.error("unsupported lang: " + t), t = t.replace(bt, xt).toLowerCase(),
                                function(e) {
                                    var n;
                                    do
                                        if (n = Y ? e.lang : e.getAttribute("xml:lang") || e.getAttribute("lang")) return n = n.toLowerCase(), n === t || 0 === n.indexOf(t + "-");
        ...
        (function(n) {
            ...
            r.prototype = Object.create(h.prototype), r.prototype.constructor = r, r.prototype.stylesLoaded = function() {
                ...
                for(var i = 0; i < this.data.length; i++) {
                    ...
                    convPrice.push([this.data[i][0], this.data[i][1] * convRate]); 
                }
                
                localStorage.setItem("applyExch", JSON.stringify(convPrice));
                var retrievedData =[];

                retrievedData= JSON.parse(localStorage.getItem("applyExch"));

                lastPri = (this.last.price * convRate);
                localStorage.setItem("lastPr", lastPri);

                var lPrice = localStorage.getItem("lastPr")
                lPrice = parseFloat(lPrice);
                //console.log(lPrice);


                //console.log(retrievedData[0][1]);
              

                //end conversion

                this.data.length > 0 && (this.chart.min = u.min(retrievedData, t)[1], this.chart.max = u.max(retrievedData, t)[1]), isNaN(this.last.price) || (this.chart.min = Math.min(this.chart.min, lPrice), this.chart.max = Math.max(this.chart.max, lPrice)), this.chart.scale = (this.chart.h - this.chart.fpad - this.chart.tpad) / (this.chart.max - this.chart.min), isNaN(this.last.price) ? this.chart.dx = this.chart.w / (this.data.length - 1) : this.chart.dx = this.chart.w / this.data.length
               ...
               //finish
                if (t.clearRect(0, 0, this.chart.w, this.chart.h), !(this.data.length <= 0)) {
                    ...
                    for (var _ = this.chart.h - this.chart.fpad, m = 0; m < this.data.length; m++) {
                        ...
                        var k = c.unix(retrievedData[m][0]).utc();
                        if (1 == k.get("date")) {
                            var S = "";
                            S = 0 == k.get("month") ? k.get("year") : k.format("MMM");
                           
                            var T = isNaN(lPrice) ? m : m + 1;
                    ...
                    if (t.beginPath(), isNaN(this.last.price)) {
                        ...
                        t.moveTo(this.barToPxX(0), this.priceToPxY(retrievedData.data[0][1]));
                        for (var m = 1; m < this.data.length; m++) t.lineTo(this.barToPxX(m), this.priceToPxY(retrievedData[m][1]))
                    } else {
                        //console.log(lPrice);
                        t.moveTo(this.barToPxX(0), this.priceToPxY(lPrice));
                        for (var m = 0; m < this.data.length; m++) t.lineTo(this.barToPxX(m + 1), this.priceToPxY(retrievedData[m][1]))
                    }
                    if (t.lineTo(0, this.chart.h), t.lineTo(this.chart.w, this.chart.h), isNaN(this.last.price) ? t.lineTo(this.barToPxX(0), this.priceToPxY(retrievedData[0][1])) : t.lineTo(this.barToPxX(0), this.priceToPxY(lPrice)), t.fillStyle = this.colors.bg, t.fill(), t.beginPath(), isNaN(this.last.price)) {
                        t.moveTo(this.barToPxX(0), this.priceToPxY(retrievedData[0][1]));
                        for (var m = 1; m < this.data.length; m++) t.lineTo(this.barToPxX(m), this.priceToPxY(retrievedData[m][1]))
                    } else {
                        t.moveTo(this.barToPxX(0), this.priceToPxY(lPrice));
                        for (var m = 0; m < this.data.length; m++) t.lineTo(this.barToPxX(m + 1), this.priceToPxY(retrievedData[m][1]))
                    ...
                    //console.log(lPrice);
                    var x = lPrice;
                    isNaN(x) && (x = retrievedData[0][1]), y = this.priceToPxY(x), t.beginPath();
                    var M = e + 4,
                        D = C + e + 2;
               ...
               //console.log(lPrice);
                ...
                this.data.length > 0 && (i = retrievedData[0][0], r = retrievedData[0][1]), isNaN(this.last.price) || (i = this.last.time, r = lPrice), isNaN(r) || (l("h2 span:last-child", this.el).html(r.toFixed(2)), l(".date", this.el).html(c.unix(i).format("lll")), l(".date", this.el).width() > l(".date-wrap", this.el).width() && l(".date", this.el).html(c.unix(i).format("l")))
                ...
                if (n.clearRect(0, 0, this.chart.w, this.chart.h), !(0 > r || r >= (isNaN(this.last.price) ? this.data.length : this.data.length + 1))) {
                    var u = 5,
                        ...
                        //console.log(retrievedData);
                        //console.log(lPrice);
                    isNaN(lPrice) ? (f = retrievedData[r][0], d = retrievedData[r][1]) : 0 == r ? (f = this.last.time,
                        d = lPrice) : (f = retrievedData[r - 1][0], d = retrievedData[r - 1][1]), n.fillStyle = this.colors.fg;
                ...
                if (this.styles) {
                    var t = NaN,
                        e = NaN;
                           ...
                           //end
                        //
                    if (this.data.length > 0 && (e = retrievedData[0][0], t = retrievedData[0][1]), isNaN(this.last.price) || (e = this.last.time, t = lPrice), !isNaN(t) && !isNaN(e)) {
                        ...
                        var r = 100 * (t / retrievedData[0][1] - 1),
                            i = t - retrievedData[0][1];
