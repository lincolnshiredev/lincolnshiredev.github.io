    ...
    $(document).ready(function() {
        ...
        if (plugins.btcCalculator.length) {
            ...
            $.getJSON("https://blockchain.info/ticker", function(btcJsonData) {
				...
				//////// Function to move GBP to first position in the array ///////

				//function: currencyList.move(move_from, move_to) - move_from and to are the array indexs
				//e.g. currencyList.move(1,2) swaps the value from position 1 to position 2

				Array.prototype.move = function(move_from, move_to) {
					this.splice(move_to, 0, this.splice(move_from, 1)[0]);
				};

				//call the fuction
				currencyList.move(9,0) //move GBP from position 9 in array to position 0
				currencyList.move(9,1) // Move EUR into pos 1 in array
			
				/////// End of of movefunction ////////

				/// Function to add flags to the drop down menu (function is called in btcFormCurrencySelect )

				function formatState (opt) {
		             //create imagesrc url from currency name 
					var imgurl = "images/flags/" + opt.text.toLowerCase() + ".png"
					    //create custom template for select2 js plugin and return the value of the html
						var $opt = $(
							'<span><img src="' + imgurl + '" width="20px"/> ' +" " + opt.text.toUpperCase() + '</span>'
						 );
						 return $opt;
				};
				...
				for (var i = 0; i < plugins.btcCalculator.length; i++) {
					...
					btcFormCurrencySelect.select2({
						...
						//these two attributes call the format state function to return the flags
